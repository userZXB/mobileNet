...
# MOBILENETV1_CONV_DEFS specifies the MobileNet body 和官网一样，已经对比
...
# 将整个网络划分为三层，1）标准卷积层 2）深度卷积层 3）点卷积层
# 这样整个scope就清楚了
# 其中返回的endpoint可以用于额外的用途，比如单独拿出计算每一层的损失值以及可视化
# 其中指定的final_endpoint用于决定什么时候训练停止
# 每一层的中间结果已经保存在了endpoints字典中
def mobilenet_v1_base(inputs,
  ...
  if conv_defs is None:
    conv_defs = MOBILENETV1_CONV_DEFS # 防止出错
    ...
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding=padding):
      ...
      for i, conv_def in enumerate(conv_defs):
        ...
        if isinstance(conv_def, Conv):
          ...
          tf.summary.image(end_point, _concact_features(net), 1)
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        if global_pool:
          ...
          # 池化层，具体操作可以查一下资料关键点在[1,2]
          net = tf.reduce_mean(net, [1, 2], keep_dims=True, name='global_pool')
          end_points['global_pool'] = net
          tf.summary.image('global_pool', net, 1)
        else:
          ...
          tf.summary.image('AvgPool_la', _concact_features(net), 1)
        if not num_classes:
          return net, end_points
        # 1 x 1 x 1024
        ...
        # 这里不应该这样做，这样做后只剩下4个通道，应该原样输出7个通道
        tf.summary.image('Conv2d_1c_1x1', _concact_logits(logits), 40)
        if spatial_squeeze:
          logits = tf.squeeze(logits, [1, 2], name='SpatialSqueeze')
      end_points['Logits'] = logits
      if prediction_fn:
        end_points['Predictions'] = prediction_fn(logits, scope='Predictions')
        # tf.summary.image('Predictions', prediction_fn(logits, scope='Predictions'), 40)
...
def mobilenet_v1_arg_scope(
  ...
  """
  # BN操作所涉及到的变量，为什么需要BN，为了防止梯度消失，加快模型收敛
  batch_norm_params = {
      'center': True,
      'scale': True,
