...
class DeploymentConfig(object):
  ...
  def __init__(self,
    ...
    """
    if num_replicas > 1: # 先判断参数是否合法，num_replicas即代表使用并行的数量
      if num_ps_tasks < 1: # 也就是说tasks一定要处于激活状态，即>1，将参数服务器和计算服务器的任务统一用num_ps_tasks代表
        raise ValueError('When using replicas num_ps_tasks must be positive')
    if num_replicas > 1 or num_ps_tasks > 0: # 有没有指定计算服务器和参数服务器
      if not worker_job_name:
        raise ValueError('Must specify worker_job_name when using replicas')
      if not ps_job_name:
        raise ValueError('Must specify ps_job_name when using parameter server')
    if replica_id >= num_replicas: # 任务的数量不可以比工作副本数量大
  ...
  def caching_device(self):
    ...
    """
    if self._num_ps_tasks > 0: # 返回储存变量的device的名字
      return lambda op: op.device
    else:
      return None

  def clone_device(self, clone_index):# 根据克隆的索引，获得
